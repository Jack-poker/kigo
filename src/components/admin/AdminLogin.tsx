...
export function AdminLogin({ className, ...props }: React.ComponentProps<'div'>) {
    ...
    tokenPromise.current = (async () => {
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          ...
          console.error(`CSRF token fetch error (attempt ${attempt}/${retries}):`, error);
          if (attempt === retries) {
            toast({
              variant: 'destructive',
              title: 'Error',
          ...
          await new Promise((resolve) => setTimeout(resolve, delay * Math.pow(1.5, attempt - 1) + jitter));
        }
      }
      return null;
    })();
  ...
  return (
    <div className={cn('min-h-screen flex items-center justify-center bg-gradient-to-b from-white to-amber-50/30 p-4', className)} {...props}>
      <div className="w-full max-w-4xl flex flex-col gap-6">
        <div className="flex justify-center mb-4">
          <img src="/assets/logo.png" alt="Kaascan logo" className="w-48 sm:w-56 md:w-64 h-auto object-contain" />
        </div>
        <Card className="overflow-hidden bg-[hsl(var(--primary))] shadow-lg w-full mx-auto">
          <CardContent className="grid p-0 md:grid-cols-2">
            <form className="p-4 sm:p-6 md:p-8" onSubmit={(e) => e.preventDefault()}>
              <div className="flex flex-col gap-6">
                <div className="flex flex-col items-center text-center">
                  <h1 className="text-xl sm:text-2xl font-bold text-white">Admin Login</h1>
                  <p className="text-sm sm:text-base text-white/90">Login to your Kaascan Admin account</p>
                </div>
                {isFetchingToken && !effectiveCsrfToken && (
                  <p className="text-center text-sm text-white/90">Fetching token...</p>
                )}
                ...
                <div className="text-center text-xs sm:text-sm text-white/90">
                  Need assistance?{' '}
                  <a href="/support" className="text-amber-400 hover:text-amber-500 underline underline-offset-4">
                    Contact support
                  </a>
                </div>
              </div>
            </form>
            <div className="relative hidden md:block bg-white h-full">
              ...
              />
              <div className="absolute bottom-4 sm:bottom-6 md:bottom-8 left-4 sm:left-6 md:left-8 right-4 sm:right-6 md:right-8 z-20 text-white">
                <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-1 sm:mb-2">Welcome to Kaascan Admin Portal</h2>
                <p className="text-sm sm:text-base md:text-lg opacity-90">
                  Manage school payments, ads, and parent accounts with ease and security.
                </p>
              </div>
            </div>
          </CardContent>
        </Card>
        <div className="text-center text-xs sm:text-sm text-muted-foreground [&_a]:underline [&_a]:underline-offset-4 hover:[&_a]:text-primary">
